        ...
        if (amount >= 0) {
            if (units.includes(firstUnit) && units.includes(secondUnit)) {
                switch (firstUnit + "|" + secondUnit) {
                    ...
                    default:
                        return console.log("Error! Please enter two different units.");
                };
            } else {
                return console.log("Error! We do not support that unit yet. Please enter either: cm or m.");
            };
        } else {
            return console.log("Error! Please enter an amount greater or equal to 0.");
        };
    } else {
        return console.log("Error! Please enter in the format (X unit1 in unit2).");
    };
};