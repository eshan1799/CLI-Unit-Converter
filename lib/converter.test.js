...
describe("testing converter function", () => {
    ...
    it("display error when input is formatted incorrectly", () => {
        const expected = console.log("Error! Please enter in the format (X unit1 in unit2).");
        const inputted = converter("100cm in m");
        expect(inputted).to.equal(expected);
    });
});